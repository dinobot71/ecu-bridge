...
/**
 * 
 * db \ PgSQLField - this helper class manages the type conversion
...
/**
 * 
 * class PgSQLField - this is lightweight object is used to
...
class littlemdesign_db_PgSQLField {
    ...
    if($this->type == "text") {
      ...
      if($this->fixed) {
      	...
      	if(empty($this->length)) {
      	  ...
      	  /* 
      	   * PostgreSQL likely allows bigger fixed strings, but
      	   * lets be consistent with MySQL.
      	   * 
      	   */
      	...
      	if(empty($this->length)) {
      	  ...
      	  /* 
      	   * again, PostgreSQL can support much bigger 
      	   * VARCHAR lengths...but lets be consistent 
      	   * with MySQL.
      	   * 
      	   */
      ...
      /*
       * NOTE: PostgreSQL does not specifically have 
      ...
      if (!empty($this->length)) {
        ...
        if ($this->length <= 2) {
          $this->decl = 'SMALLINT';
        } else if ($this->length == 3 || $this->length == 4) {
          $this->decl = 'INT';
      ...
      $this->decl = 'NUMERIC('.$this->length.','.$this->scale.')';
      ...
      $this->decl = 'TIME without time zone';
      ...
      $this->decl = 'TIMESTAMP without time zone';
...
/*
$f = new littlemdesign_db_PgSQLField(
    'myfield',
    'text', 
    $length   = '1', 
    $unsigned = false, 
