...
$db  = new littlemdesign_db_MySQL($dsn); 
if(!$db->isReady()) {
...
$result = $db->selectDatabase('wordpress');
if($result === false) {
...
echo ". setting database...\n";
$result = $db->listTables('wordpress');
if($result === false) {
  echo "Can not set current databases: ".$db->getError()."\n";
  exit(1);
}
...
echo ". checking...\n";
$result = $db->tableExists('dept_anager');
...
echo ". checking...\n";
$result = $db->databaseExists('rolermarathon');
